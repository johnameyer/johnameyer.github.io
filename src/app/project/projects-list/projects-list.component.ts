...
const dateOrdering = (a: StandaloneProject, b: StandaloneProject) =>
  a.endDate == 'Current'
    ? 1
    : b.endDate == 'Current'
      ? -1
...
export class ProjectsListComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  ordering$: BehaviorSubject<(a: Project, b: Project) => any> = new BehaviorSubject(
    (a: StandaloneProject, b: StandaloneProject) => -dateOrdering(a, b),
  );
  technologies: string[];
  ...
  ngOnInit() {
    this.updateItems();
    this.selectedTechnology.valueChanges
  ...
  updateItems() {
    ...
    if (this.selectedTechnology.value != '' && this.technologies.includes(this.selectedTechnology.value)) {
      selectedProjects = selectedProjects.filter(project =>
        project.technologies?.includes(this.selectedTechnology.value),
      );
    }
  ...
  search = (text$: Observable<string>) => {
    ...
    return merge(debouncedText$, inputFocus$, clicksWithClosedPopup$).pipe(
      map(term =>
        (term === ''
          ? this.technologies
          : this.technologies.filter(v => v.toLowerCase().indexOf(term.toLowerCase()) > -1)
        ).slice(0, 10),
      ),
    );
  };
